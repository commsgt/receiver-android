...
import org.opendroneid.android.data.*;
...
public class OpenDroneIdDataManager {
    private static final Logger LOGGER = Logger.getLogger(OpenDroneIdDataManager.class.getName());
    public final ConcurrentHashMap<Long, AircraftObject> aircraft = new ConcurrentHashMap<>();
    public GpsLocation receiverLocation;
    ...
    public ConcurrentHashMap<Long, AircraftObject> getAircraft() { return aircraft; }
    ...
    public void receiveDataBluetooth(byte[] data, String macAddress, int rssi, long timestampNanos,
        ...
        OpenDroneIdParser.Message<?> message =
                OpenDroneIdParser.parseData(data, 2, timestampNanos, logMessageEntry, receiverLocation);
        if (message == null) return;
        receiveData(timestampNanos, macAddress, macAddressLong, rssi, message, logMessageEntry, transportType);
    }
    ...
    void receiveData(long timeNano, String macAddress, long macAddressLong, int rssi,
        ...
        if (message.header.type == OpenDroneIdParser.Type.MESSAGE_PACK)
            handleMessagePack(ac, (OpenDroneIdParser.Message<OpenDroneIdParser.MessagePack>) message,
                    timeNano, logMessageEntry, message.msgCounter);
        else
    ...
    private void handleMessages(AircraftObject ac, OpenDroneIdParser.Message<?> message) {
        switch (message.header.type) {
            case BASIC_ID:
                handleBasicId(ac, (OpenDroneIdParser.Message<OpenDroneIdParser.BasicId>) message); break;
            case LOCATION:
                handleLocation(ac, (OpenDroneIdParser.Message<OpenDroneIdParser.Location>) message); break;
            case AUTH:
                handleAuthentication(ac, (OpenDroneIdParser.Message<OpenDroneIdParser.Authentication>) message); break;
            case SELFID:
                handleSelfID(ac, (OpenDroneIdParser.Message<OpenDroneIdParser.SelfID>) message); break;
            case SYSTEM:
    ...
    private void handleBasicId(AircraftObject ac, OpenDroneIdParser.Message<OpenDroneIdParser.BasicId> message) {
        ...
        } else if (type2 == Identification.IdTypeEnum.None || type2 == data.getIdType()) {
            ac.identification2.setValue(data);
        } else {
            LOGGER.info("Discarded Basic ID of type: " + data.getIdType() +
                    ". Already have " + type1 + " and " + type2);
        }
    }
    ...
    private void handleMessagePack(AircraftObject ac, OpenDroneIdParser.Message<OpenDroneIdParser.MessagePack> message,
        ...
        if (raw.messageSize != Constants.MAX_MESSAGE_SIZE
                ...
                || raw.messagesInPack > Constants.MAX_MESSAGES_IN_PACK) return;
