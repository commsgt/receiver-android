    ...
    public StringBuilder getMessageLogEntry() {
        ...
        for (int j = 0; j < 2; j++) {
            if (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.BASIC_ID) {
                ...
                i++;
            } else { entry.append(DELIM_BASIC_ID); }
        }
        while (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.BASIC_ID) i++;
        ...
        if (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.LOCATION) {
            ...
            entry.append(message.payload.toCsvString());
            i++;
        } else { entry.append(DELIM_LOCATION); }
        while (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.LOCATION) i++;
        while (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.AUTH) i++;
        ...
        if (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.SELFID) {
            ...
            entry.append(message.payload.toCsvString());
            i++;
        } else { entry.append(DELIM_SELF_ID); }
        while (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.SELFID) i++;
        ...
        if (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.SYSTEM) {
            ...
            entry.append(message.payload.toCsvString());
            i++;
        } else { entry.append(DELIM_SYSTEM); }
        while (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.SYSTEM) i++;
        ...
        if (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.OPERATOR_ID) {
            ...
            entry.append(message.payload.toCsvString());
        } else { entry.append(DELIM_OPERATOR); }
        ...
        for (int j = 0; j < Constants.MAX_AUTH_DATA_PAGES; j++) {
            if (i < messages.size() && messages.get(i).header.type == OpenDroneIdParser.Type.AUTH) {
                ...
                if (message.payload.getAuthDataPage() == j) {
                    entry.append(message.payload.toCsvString());
                    i++;
                } else { entry.append(DELIM_AUTHENTICATION); }
            } else { entry.append(DELIM_AUTHENTICATION); }
        }
        return entry;
    }
